...
def monitor_log_and_respond():
    ...
    """
    # 추가된 Beta Terms of Use 프롬프트도 감지
    prompts_remaining = [
        ...
        while True:
            ...
            if new_data:
                ...
                if account_prompt in buffer:
                    ...
                    send_to_screen("y")
                    # account_prompt 다음 내용으로 버퍼 자르기
                    buffer = buffer.split(account_prompt, 1)[1]

                # 버퍼에 줄바꿈이 있다면 한 줄씩 처리
                while "\n" in buffer:
                    ...
                    # 모든 자동 응답이 끝나면 스크립트 종료
                    if not prompts_remaining:
...
def main():
    ...
    print("자동화 스크립트가 종료됩니다. screen -r nexus 로 접속하여 확인할 수 있으며, 종료 시 Ctrl+A+D")
